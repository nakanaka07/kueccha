...
import { MAP_CONFIG, AREA_COLORS, AREAS } from "./appConstants"; // AREAS をインポート
        ...
        >
            <MarkerClusterer>
                {(clusterer) => (
                    <>
                        {pois.map((poi) => {
                            const areaName = AREAS[poi.area]; // <- areaName を取得
                            const color = AREA_COLORS[areaName] || "#000000"; // <- areaName を使用して色を取得
                            ...
                            return (
                                <Marker
                                    ...
                                    icon={{
                                        ...
                                        fillColor: color, // <- ここで color を使用
                                        fillOpacity: 1,
                                        strokeColor: color, // <- ここで color を使用
                                        strokeWeight: 2,
                                        scale: 12,
                                    }}
                                />
