...
export function useSheetData(areas: AreaType[]): UseSheetDataResult {
    ...
    useEffect(() => {
        ...
        const areasToFetch = areas.filter(
            (area) => !poiCache.current.has(JSON.stringify(area))
        );

        const loadData = async () => {
            setIsLoading(true);
            try {
							if (areasToFetch.length === 0) { //<- Early Return
								const allPois = areas.flatMap(
										(area) => poiCache.current.get(JSON.stringify(area)) ?? []
								);
								setPois(allPois);
								setIsLoading(false);
								return;
                ...
                const newPoiData = await Promise.all(
                    areasToFetch.map(async (area) => {
                        ...
                        if (!response.ok) {
                            ...
                            const httpErrorMessage = `HTTPエラー: データの取得に失敗しました。ステータス: ${response.status} ${errorData?.error?.message}`;
                            console.error(httpErrorMessage, errorData);
                            throw new Error("データの取得に失敗しました。");
                        }
                        const data = await response.json();
                        if (!data.values || !Array.isArray(data.values)) {
                            const dataErrorMessage = `データ形式が不正です: ${AREAS[area]}。データ: ${JSON.stringify(
                                data
                            )}`;
                        ...
                        return data.values
                            .slice(1)
                            .map((row: any[]) => transformRowToPoi(row, area)); // any[] to handle potential numbers
                    })
                );

                newPoiData.forEach((data, index) => {
                    poiCache.current.set(JSON.stringify(areasToFetch[index]), data);
                });
