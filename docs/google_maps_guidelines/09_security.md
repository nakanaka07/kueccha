...
function verifySRI(): void {
  ...
  const mapsScripts = document.querySelectorAll('script[src*="googleapis.com"]');
...
function setupTokenBasedAuth(): void {
  ...
  function scheduleTokenRefresh(): void {
    if (tokenExpiry) {
      const timeUntilExpiry = Math.max(0, tokenExpiry - Date.now() - 600 * 1000); // 期限の10分前
  ...
  async function refreshApiAccessToken(): Promise<void> {
    try {
      ...
      if (newExpiry) {
        const timeUntilNextRefresh = Math.max(0, newExpiry - Date.now() - 600 * 1000);
