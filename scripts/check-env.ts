...
const ENV_CATEGORIES: EnvCategory[] = [
  {
    ...
    variables: [
      ...
      { name: 'VITE_GOOGLE_MAPS_MAP_ID', required: true, description: 'カスタムマップスタイルのID' }
    ]
  },
  {
    name: 'Google Sheets API',
    variables: [
      { name: 'VITE_GOOGLE_SHEETS_API_KEY', required: true, description: 'Sheets APIアクセス用のキー' },
      { name: 'VITE_GOOGLE_SPREADSHEET_ID', required: true, description: 'データ取得元のスプレッドシートID' }
    ]
  },
  {
    name: 'EmailJS',
  ...
  },
  {
    name: 'アプリ設定',
    variables: [
      { name: 'VITE_APP_TITLE', required: false, description: 'アプリケーションのタイトル', defaultValue: 'Kueccha App' },
      { name: 'VITE_DEFAULT_ZOOM', required: false, description: '地図の初期ズームレベル', defaultValue: '13' },
      { name: 'VITE_DEFAULT_CENTER_LAT', required: false, description: '地図の初期中心位置（緯度）', defaultValue: '35.6812' },
      { name: 'VITE_DEFAULT_CENTER_LNG', required: false, description: '地図の初期中心位置（経度）', defaultValue: '139.7671' }
    ]
  }
];
 ...
 */
function loadEnvironmentFiles(): { currentEnv: Record<string, string>, exampleEnv: Record<string, string> } {
    ...
    category.variables.forEach(variable => {
      ...
      if (!envValue) {
        if (variable.required) {
          ...
          console.log(chalk.yellow(`警告: ${variable.name} が設定されていません。デフォルト値 "${variable.defaultValue}" が使用されます。`));
          hasWarnings = true;
          categoryHasIssues = true;
        } else {
          // オプションだが、設定を推奨
    ...
    if (categoryHasIssues) {
      ...
      if (missingRequired.length > 0) {
        ...
        missingRequired.forEach(varName => {
          ...
          console.error(chalk.red(`  - ${varName}${variable?.description ? ': ' + variable.description : ''}${exampleValue}`));
        });
      }
      ...
      if (missingOptional.length > 0) {
        ...
        missingOptional.forEach(varName => {
          const variable = category.variables.find(v => v.name === varName);
          const exampleValue = exampleEnv[varName] ? ` (例: ${exampleEnv[varName]})` : '';
          console.log(chalk.yellow(`  - ${varName}${variable?.description ? ': ' + variable.description : ''}${exampleValue}`));
        });
      }
    }
  });
