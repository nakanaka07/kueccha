 ...
 * - パフォーマンス最適化（キャッシュ機能）
 *
 * Google Maps統合ガイドラインおよび環境変数管理ガイドラインに準拠しています。
 */

import { LogLevel } from './logger';

// 環境変数の型定義をインポート
import type { EnvironmentConfig, LogLevelType } from '../types/env';
...
// ロガーのインスタンスを保持する変数（初期化時は簡易ロガー）
// ロガー使用ガイドラインに準拠: コンソールには直接書き込まず、ロギング層を通して行う
let envLogger = {
  error: (message: string, ...args: unknown[]) => {
    // ESLint no-console警告を避けるためにグローバルオブジェクト経由でアクセス
    // ロガー初期化時のみの一時的な使用なのでこの方法を採用
...
export function getEnvVar<T = string>({
  ...
  return cachedValue(
    ...
    () => {
      ...
      if (value === undefined) {
        const severity = critical ? 'error' : 'warn';
...
export const validateGoogleMapsConfig = (): { isValid: boolean; messages: string[] } => {
  ...
  if (apiKey.length > 0 && !apiKeyRestrictions && isProdEnvironment()) {
...
export function checkEnvironment(verbose: boolean = false): void {
  ...
  // 詳細な検証ルール（Google Maps統合ガイドラインに準拠）
  ...
  if (isValid && detailedValid && (verbose || isDevEnvironment())) {
    envLogger.info('✅ 環境変数の検証が完了しました。必要な環境変数はすべて設定されています。', {
      component: 'EnvCheck',
      action: 'validation_success',
    });
