...
export function useMapMarkers({
  ...
  const createMarkers = useCallback(() => {
    ...
    return newMarkers;
  }, [mapRef, pois, createMarker, enableClustering, clusterOptions, cleanupMarkers]); // マップもしくはPOIデータが変更されたらマーカーを更新
  useEffect(() => {
    // マップが存在する場合のみマーカーを作成
    if (mapRef.current) {
      createMarkers();
    }
    // コンポーネントのアンマウント時またはマーカー更新時にクリーンアップ
    return cleanupMarkers;
    // 依存配列に必要なすべての依存関係を含める
  }, [createMarkers, cleanupMarkers, mapRef]);
