...
import { APP, INITIAL_VISIBILITY } from '../utils/constants';
import type { AreaType, AreaVisibility } from '../utils/types';
...
export const useAreaVisibility = (persistToStorage = true) => {
  ...
  const getSavedVisibility = useCallback((): AreaVisibility => {
    ...
    try {
      ...
      if (saved) {
        ...
        const allKeysExist = Object.keys(APP.areas).every((area) => typeof parsed[area] === 'boolean');
  ...
  const showAllAreas = useCallback(() => {
    const allVisible = Object.keys(APP.areas).reduce((acc, area) => ({ ...acc, [area]: true }), {} as AreaVisibility);
    setAreaVisibility(allVisible);
  }, []);

  // すべてのエリアを非表示にする関数
  const hideAllAreas = useCallback(() => {
    const allHidden = Object.keys(APP.areas).reduce((acc, area) => ({ ...acc, [area]: false }), {} as AreaVisibility);
