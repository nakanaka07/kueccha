...
import { AREAS } from "./appConstants"; // AREAS をインポート
...
const InfoWindowContent = memo(({ poi }: { poi: Poi }) => {
	...
	const additionalInfo = useMemo(() => {
        const areaDisplayName = AREAS[poi.area] || poi.area; // AREAS から表示名を取得。 存在しない場合は poi.area をそのまま使用
		return [
            ...
            { label: "表示シート", value: areaDisplayName }, // 取得した表示名を使用
		].filter(({ value }) => !!value);
	}, [poi]);
				...
				<div>
					{businessHours.map(({ day, hours }, index) => (
						<div key={index}> {/* hours-row のクラス名を削除 */}
							<span><strong>{day} : </strong></span> {/* day のクラス名を削除 */}
				...
				<div>
					{additionalInfo.map(({ label, value }, index) => (
						<div key={index}> {/* info-row のクラス名を削除 */}
							<span><strong>{label} : </strong></span> {/* label のクラス名を削除 */}
