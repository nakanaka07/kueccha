...
import './InfoWindowContent.css'; // CSSファイルをインポート
...
const InfoWindowContent = memo(({ poi }: { poi: Poi }) => {
	...
	const informationLinks = useMemo(
		...
		[poi.information]
	); // poi.informationが変更された場合のみ再計算
	const viewLinks = useMemo(
		() => convertUrlsToLinks(poi.view, "Googleマップで見る"),
		[poi.view]
	); // poi.viewが変更された場合のみ再計算

	return (
		<div className="info-window">
            <div className="title-bar"> {/* Flexboxコンテナ */}
                <h4 style={{marginTop:"0"}}>{poi.name}</h4> {/* h4 のスタイルをリセット */}
            </div>
			{/* 営業時間 */}
